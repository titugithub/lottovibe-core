			...
			if ( isset( $_POST['plugin'] ) ) {
				...
				if ( 'plugin' === $type ) {
					...
					if ( ! current_user_can( 'activate_plugins' ) ) {
						wp_send_json_error( esc_html__( 'Plugin activation is disabled for you on this site.', 'rts-header-footer-elementor' ) );
					}
					...
					if ( ! is_wp_error( $activate ) ) {
						...
						wp_send_json_success( esc_html__( 'Plugin Activated.', 'rts-header-footer-elementor' ) );
					}
				}
				...
				if ( 'theme' === $type ) {
					...
					if ( ! ( current_user_can( 'switch_themes' ) ) ) {
						wp_send_json_error( esc_html__( 'Theme activation is disabled for you on this site.', 'rts-header-footer-elementor' ) );
					}
					...
					if ( ! is_wp_error( $activate ) ) {
						...
						wp_send_json_success( esc_html__( 'Theme Activated.', 'rts-header-footer-elementor' ) );
					}
				}
			}

			if ( 'plugin' === $type ) {
				wp_send_json_error( esc_html__( 'Could not activate plugin. Please activate from the Plugins page.', 'rts-header-footer-elementor' ) );
			} elseif ( 'theme' === $type ) {
				wp_send_json_error( esc_html__( 'Could not activate theme. Please activate from the Themes page.', 'rts-header-footer-elementor' ) );
			}
		}
